...
func (h *Handler) RegisterRoutes(api *gin.RouterGroup) {
	...
	api.GET("/platform/admins", h.listPlatformAdmins)
	api.POST("/platform/admins", h.createPlatformAdmin)
	api.PUT("/platform/admins/:id", h.updatePlatformAdmin)
	api.POST("/platform/admins/:id/status", h.setPlatformAdminStatus)
}
...
type platformOverviewView struct {
	ActiveOrganizationCount   int `json:"activeOrganizationCount"`
	AvailableInviteCount      int `json:"availableInviteCount"`
	DisabledOrganizationCount int `json:"disabledOrganizationCount"`
	ExhaustedInviteCount      int `json:"exhaustedInviteCount"`
	OrganizationCount         int `json:"organizationCount"`
	PendingRegistrationCount  int `json:"pendingRegistrationCount"`
}
...
func (r createPlatformAdminRequest) Validate() []response.ValidationDetail {
	...
	if strings.TrimSpace(r.RealName) == "" {
		details = append(details, response.ValidationDetail{Field: "realName", Reason: "required"})
	}
	if r.Status != "" && r.Status != string(identity.UserStatusActive) && r.Status != string(identity.UserStatusDisabled) {
...
func (r updatePlatformAdminRequest) Validate() []response.ValidationDetail {
	...
	if r.Status != string(identity.UserStatusActive) && r.Status != string(identity.UserStatusDisabled) {
		details = append(details, response.ValidationDetail{Field: "status", Reason: "invalid_value"})
	}
	return details
}
...
func (h *Handler) overview(c *gin.Context) {
	...
	organizations, err := h.store.ListOrganizations(c.Request.Context())
	if err != nil {
		h.respondError(c, err)
		return
	}
	invites, err := h.store.ListInvites(c.Request.Context(), "")
	if err != nil {
		h.respondError(c, err)
		return
	}
	registrations, err := h.store.ListRegistrations(c.Request.Context(), RegistrationStatusPending)
	if err != nil {
		h.respondError(c, err)
		return
	}
	result := platformOverviewView{OrganizationCount: len(organizations), PendingRegistrationCount: len(registrations)}
	for _, item := range organizations {
		switch item.Status {
		case OrganizationStatusActive:
			result.ActiveOrganizationCount++
		case OrganizationStatusDisabled:
			result.DisabledOrganizationCount++
		}
	}
	for _, item := range invites {
		switch item.Status {
		case InviteStatusActive:
			result.AvailableInviteCount++
		case InviteStatusExhausted:
			result.ExhaustedInviteCount++
		}
	}
...
func (h *Handler) listOrganizations(c *gin.Context) {
	...
	for _, item := range items {
		...
		if keyword != "" && !strings.Contains(strings.ToLower(item.Name), keyword) && !strings.Contains(strings.ToLower(item.Slug), keyword) && !strings.Contains(strings.ToLower(item.ContactName), keyword) {
			continue
		}
...
func (h *Handler) setOrganizationAuthorization(c *gin.Context) {
	if _, ok := platformPrincipal(c); !ok {
		return
	}
	id, ok := parseID(c)
	...
	var authorizedUntil *time.Time
	if value := strings.TrimSpace(req.AuthorizedUntil); value != "" {
		parsed, err := time.ParseInLocation("2006-01-02", value, time.UTC)
		if err != nil {
			response.Error(c, response.ValidationFailed([]response.ValidationDetail{{Field: "authorizedUntil", Reason: "date_format"}}))
			return
		}
		authorizedUntil = &parsed
	}
	if err := h.store.SetOrganizationAuthorization(c.Request.Context(), id, authorizedUntil); err != nil {
		h.respondError(c, err)
		return
	}
	response.OK(c, true)
...
func (h *Handler) listPlatformAdmins(c *gin.Context) {
	...
	keyword := strings.ToLower(strings.TrimSpace(c.Query("keyword")))
	status := strings.TrimSpace(c.Query("status"))
	out := make([]platformAdminView, 0)
	for _, user := range users {
		if !identity.IsPlatformAdmin(user.Role) {
			continue
		}
		...
		out = append(out, toPlatformAdminView(user))
	}
	response.OK(c, gin.H{"items": out, "total": len(out)})
}
...
func (h *Handler) createPlatformAdmin(c *gin.Context) {
	...
	if _, err := h.users.FindUserByUsername(c.Request.Context(), username); err == nil {
		response.Error(c, response.BadRequest("用户名已存在，请换一个账号", identity.ErrUsernameTaken))
		return
	} else if !errors.Is(err, identity.ErrUserNotFound) {
		response.Error(c, response.Internal(err))
		return
	...
	status := identity.UserStatus(req.Status)
	if status == "" {
		status = identity.UserStatusActive
	}
	user, err := h.users.CreateUser(c.Request.Context(), identity.CreateUserParams{
		OrganizationID: identity.DefaultOrganizationID,
		Username:       username,
		PasswordHash:   string(hash),
		Role:           identity.UserRolePlatformAdmin,
	...
	response.Created(c, "/api/v1/platform/admins/"+strconv.FormatUint(user.ID, 10), toPlatformAdminView(user))
}
...
func (h *Handler) updatePlatformAdmin(c *gin.Context) {
	...
	if !identity.IsPlatformAdmin(current.Role) {
		response.Error(c, response.NotFound())
		return
	}
	if id == principal.SubjectID && strings.TrimSpace(req.Status) == string(identity.UserStatusDisabled) {
		response.Error(c, response.BadRequest("不能停用当前登录账号", nil))
		return
	}
	...
	if err := h.users.UpdateUser(c.Request.Context(), identity.UpdateUserParams{
		ID:           id,
	...
	response.OK(c, toPlatformAdminView(updated))
}

func (h *Handler) setPlatformAdminStatus(c *gin.Context) {
	principal, ok := platformPrincipal(c)
	if !ok {
		return
	}
	...
	if id == principal.SubjectID && status == identity.UserStatusDisabled {
		response.Error(c, response.BadRequest("不能停用当前登录账号", nil))
		return
	}
	current, err := h.users.FindUserByID(c.Request.Context(), id)
	...
	if err := h.users.SetUserStatus(c.Request.Context(), identity.SetUserStatusParams{ID: id, Status: status}); err != nil {
		h.respondError(c, err)
		return
	}
	response.OK(c, true)
...
func toPlatformAdminView(user identity.User) platformAdminView {
	return platformAdminView{ID: user.ID, Username: user.Username, RealName: user.Nickname, Role: string(user.Role), Status: string(user.Status), CreatedAt: user.CreatedAt.Format(time.RFC3339)}
}
