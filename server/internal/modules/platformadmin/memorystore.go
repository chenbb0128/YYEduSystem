...
func (s *MemoryStore) SetOrganizationAuthorization(_ context.Context, id uint64, authorizedUntil *time.Time) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	for index := range s.organizations {
		if s.organizations[index].ID == id {
			s.organizations[index].AuthorizedUntil = cloneTime(authorizedUntil)
			s.organizations[index].UpdatedAt = time.Now().UTC()
			return nil
		}
	}
