...
// DefaultOrganizationID is the compatibility tenant used by local bootstrap
// accounts. Platform accounts are still separated by role, because the
// current schema requires users.organization_id to be non-null.
const DefaultOrganizationID uint64 = 1

const defaultOrganizationID = DefaultOrganizationID
